...
- **`/portal` middleware (2026-04-05):** Middleware must **not** import `@/auth` with **PrismaAdapter** — Edge runtime cannot load Prisma; that caused **500** on `/portal` and `/admin` before login. **`src/auth.config.ts`** holds edge-safe config; **`src/middleware.ts`** uses `NextAuth(authConfig)` only; **`src/auth.ts`** merges `authConfig` + adapter for Node.
