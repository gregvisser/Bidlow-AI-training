...
- **Custom domain `bidlow.co.uk` (2026-04-03):** **Not bound yet** in App Service — Azure requires GoDaddy DNS updates first (exact TXT/A/CNAME values in **`docs/DEPLOYMENT_AZURE.md`**). Public DNS currently routes **`www`** elsewhere (Azure Static Web Apps); apex **A** records are not this app’s inbound IP. **`AUTH_URL` / `APP_BASE_URL`** remain the default **`https://bidlow-ai-training-prod.azurewebsites.net`** until TLS on the custom host is live.
