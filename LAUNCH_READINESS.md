...
- **Browser verification — learner (2026-04-06):** On **`https://www.bidlow.co.uk`**, **`curl`** confirmed **200** for `/`, `/api/health`, `/api/ready`, `/pricing`, `/login`; **`/api/health`** body `status: ok`; **`/api/ready`** body `ready: true` with `database: up`, `blobStorage: configured`. **Learner** **`learner@bidlow.co.uk`**: sign-in reaches **`/portal`** without a redirect loop. **`/portal/courses`** shows **Core Launch Pilot**; course **`core-launch-pilot`** and lesson **`welcome`** load; **Mark complete** **persists after reload**. **`/pricing`** and **`/portal/billing`** show **billing deferred** copy — **no** live self-serve checkout.
- **Browser verification — admin (2026-04-06):** **`greg@bidlow.co.uk`** on **`https://www.bidlow.co.uk`**: sign-in reaches **`/portal`** (Staff workspace; **no** login loop). **`/admin/ops`** loads — env readiness shows **Stripe/PayPal** secrets **not set** (checkout/webhooks disabled); DB **`SELECT 1`** connected. **`/admin/courses`** lists **Core Launch Pilot** (**PUBLISHED** · 1 module · 1 enrollment · **`core-launch-pilot`**). **Edit** opens **`/admin/courses/…/edit`** with **Pricing model: Included** and course copy that **self-serve billing is off** — **no** unexpected live checkout or payment capture in the admin UI.
