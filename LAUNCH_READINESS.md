...
- **`/register`:** Default is **invite-only** (no public self-serve signup UI) unless **`OPEN_REGISTRATION=true`** is set on the Web App. The signup form and `registerUser` action remain in the codebase for environments that enable it. Production should omit **`OPEN_REGISTRATION`** or leave it unset.
