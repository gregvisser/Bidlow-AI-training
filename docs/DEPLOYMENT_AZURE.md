...
#### Finalization pass (observed 2026-04-03)

- **Azure hostname bindings:** **`bidlow.co.uk`** and **`www.bidlow.co.uk`** are **Verified** on **`bidlow-ai-training-prod`** (`az webapp config hostname list`).
- **TLS / SNI:** ARM **`hostNameSslStates`** still show **`sslState`: `Disabled`** for both custom hostnames (no thumbprint yet). **`az webapp config ssl create`** (preview) reported **“Managed Certificate creation in progress”** but certificates did not appear under **`Microsoft.Web/certificates`** in this subscription within the session, and **`curl`** to **`https://bidlow.co.uk`** failed server certificate validation until **`curl -k`** (app code returned **200** on **`/api/health`** when forcing the App Service IP and ignoring TLS). Treat **HTTPS** as **not finished** until bindings show **SNI enabled** in the Portal or CLI.
- **Public DNS (resolver `8.8.8.8`):** **`www.bidlow.co.uk`** was still a **CNAME** to **`*.azurestaticapps.net`** (Azure Static Web Apps), **not** to **`bidlow-ai-training-prod.azurewebsites.net`**. **`bidlow.co.uk`** returned **multiple** **A** records, including **`20.105.232.16`** and other IPs — for predictable routing and managed certificate validation, use **only** the App Service inbound **A** target (or remove conflicting **A** records per GoDaddy).
- **Canonical URL / `AUTH_URL`:** Do **not** switch **`AUTH_URL`** / **`APP_BASE_URL`** to the custom domain until **HTTPS** works without certificate errors on the chosen hostname (typically **`https://www.bidlow.co.uk`**). Until then the app remains configured for the default **`*.azurewebsites.net`** origin.
- **Next operator steps:** (1) In GoDaddy, set **`www`** **CNAME** to **`bidlow-ai-training-prod.azurewebsites.net`** and reduce **apex** **A** records to the single App Service IP if required. (2) Wait for propagation; re-run **`az webapp config ssl create`** per hostname **or** use Portal **TLS/SSL settings** → **Add binding** → **Create App Service Managed Certificate**. (3) Update **`AUTH_URL`** / **`APP_BASE_URL`** and GitHub **`production`** secrets to the canonical **`https://`** URL **without** a trailing slash.
