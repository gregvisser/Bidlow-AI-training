...
- **Controlled-launch deploy (2026-04-05):** Latest production deploy run **`24010949663`** (success) includes **`getToken`-only** `/portal` middleware fix. **`https://www.bidlow.co.uk`** spot-checked: **`/portal`** (signed out) redirects to **`/login`**; home + pricing show controlled-access / billing-deferred messaging when providers are absent.
- **Production browser — learner (2026-04-06):** **`learner@bidlow.co.uk`** on **`https://www.bidlow.co.uk`**: login → **`/portal`**; **Core Launch Pilot** on **`/portal/courses`**; course + **welcome** lesson open; lesson completion **persists after reload**; **`/pricing`** and **`/portal/billing`** show billing-deferred messaging (no live checkout).
- **Production browser — admin (2026-04-06):** **`greg@bidlow.co.uk`**: login → **`/portal`** (no loop); **`/admin/ops`** and **`/admin/courses`** load; **Core Launch Pilot** visible; **Edit** opens the course editor (**Pricing model: Included**; copy still reflects billing off). Env summary on Ops still shows **no** Stripe/PayPal live keys — controlled-launch posture unchanged.
...
- **`/portal` / `/admin`:** Middleware uses **`getToken` from `next-auth/jwt`** only (no `NextAuth(authConfig)` in Edge — the bundler still pulled Prisma/pg into the middleware chunk and caused **500**). **`getToken({ secureCookie })`** must match HTTPS session cookies (`__Secure-` prefix / salt); otherwise production **login succeeds then loops** to `/login`. Unauthenticated visits should **redirect to `/login`**, not error.
- **Access model:** No **in-app invite** system. Grant access by **`npm run ops:create-user`** ( **`scripts/create-production-user.ts`** — env: `CREATE_USER_EMAIL`, `CREATE_USER_PASSWORD`, `CREATE_USER_NAME`, `CREATE_USER_ROLE` + `DATABASE_URL`), then **`npm run ops:create-min-course`** if the catalog is empty, then **`npm run ops:grant-enrollment`** for learners (**`ENROLL_USER_EMAIL`**, **`COURSE_SLUG`**) after **`npm run ops:list-courses`**, or **Google OAuth** if enabled, or raw DB access. **Included** (or unset) **`pricingModel`** + **enrollment** is enough for content without billing. Full usage: **`LAUNCH_READINESS.md`**.
