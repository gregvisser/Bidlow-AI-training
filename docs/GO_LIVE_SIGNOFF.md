...
- **Access model:** No **in-app invite** system. Grant access by **`npm run ops:create-user`** ( **`scripts/create-production-user.ts`** — env: `CREATE_USER_EMAIL`, `CREATE_USER_PASSWORD`, `CREATE_USER_NAME`, `CREATE_USER_ROLE` + `DATABASE_URL`), **Google OAuth** if enabled, or raw DB access. Enroll users in courses as your process requires; share initial credentials **out of band**. Do **not** rely on demo seed accounts in production. Full usage: **`LAUNCH_READINESS.md`** → *Manual production user — copy/paste commands*.
