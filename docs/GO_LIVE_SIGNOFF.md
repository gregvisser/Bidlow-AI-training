...
- **Access model:** No **in-app invite** system. Grant access by **`npm run ops:create-user`** ( **`scripts/create-production-user.ts`** — env: `CREATE_USER_EMAIL`, `CREATE_USER_PASSWORD`, `CREATE_USER_NAME`, `CREATE_USER_ROLE` + `DATABASE_URL`), then **`npm run ops:grant-enrollment`** for learners (**`ENROLL_USER_EMAIL`**, **`COURSE_SLUG`**) after **`npm run ops:list-courses`**, or **Google OAuth** if enabled, or raw DB access. **Included** (or unset) **`pricingModel`** + **enrollment** is enough for content without billing. Full usage: **`LAUNCH_READINESS.md`**.
